...
void RenderInsuranceContract()
{
  HVOBJECT	hPixHandle;
	...
	wprintf( sText, L"%s", pMessageStrings[ MSG_HOMEPAGE ] );
...
BOOLEAN DisplayOrderGrid( UINT8 ubGridNumber, UINT8 ubMercID )
{
  ...
  HVOBJECT hPixHandle;
	...
	wprintf( sText, L"%d", pSoldier->iTotalContractLength ); 
	...
	if( IsMercDead( ubMercID ) )
		wprintf( sText, L"%s", pMessageStrings[ MSG_LOWERCASE_NA ] ); 
	else
		wprintf( sText, L"%d", GetTimeRemainingOnSoldiersContract( pSoldier ) ); 
	...
	if( IsMercDead( ubMercID ) )
		...
		wprintf( sText, L"%d", GetTimeRemainingOnSoldiersInsuranceContract( pSoldier ) ); 
	...
	else
	{
		...
		wprintf( sText, L"%d", iCostOfContract ); 
	...
	//
	wprintf( sText, L"%d", CalculateSoldiersInsuranceContractLength( pSoldier ) );
