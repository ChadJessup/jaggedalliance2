...
void RenderAutoResolve()
{
	...
	{
		case ENEMY_ENCOUNTER_CODE:
			wprintf( str, gpStrategicString[STR_AR_ENCOUNTER_HEADER] );
			break;
		case ENEMY_INVASION_CODE:
		case CREATURE_ATTACK_CODE:
			wprintf( str, gpStrategicString[STR_AR_DEFEND_HEADER] );
			break;
	}
	...
	wprintf( str, gzLateLocalizedString[ 17 ], ubGood, ubBad );
	...
	if( gpAR->ubBattleStatus != BATTLE_IN_PROGRESS )
	{
		...
		//Render the end battle condition.
			switch( gpAR->ubBattleStatus )
			{
				case BATTLE_VICTORY:
					SetFontForeground( FONT_LTGREEN );
					wprintf( str, gpStrategicString[ STR_AR_OVER_VICTORY ] );
					break;
				case BATTLE_SURRENDERED:
				case BATTLE_CAPTURED:
					if( gpAR->ubBattleStatus == BATTLE_SURRENDERED )
					{
						wprintf( str, 50, gpStrategicString[ STR_AR_OVER_SURRENDERED ] );
					}
					else
					{
						...
						wprintf( str, 50, gpStrategicString[ STR_AR_OVER_CAPTURED ] );
					}
					SetFontForeground( FONT_RED );
					break;
				case BATTLE_DEFEAT:
					SetFontForeground( FONT_RED );
					wprintf( str, 50, gpStrategicString[ STR_AR_OVER_DEFEAT ] );
					break;
				case BATTLE_RETREAT:
					SetFontForeground( FONT_YELLOW );
					wprintf( str, 50, gpStrategicString[ STR_AR_OVER_RETREATED ] );
					break;
			}
...
void CreateAutoResolveInterface()
{
	...
	*/
	...
	for( i = 0; i < gpAR->ubCivs; i++ )
	{
		...
		wprintf( gpCivs[i].pSoldier->name, gpStrategicString[ STR_AR_MILITIA_NAME ] );
	}
	if( gubEnemyEncounterCode != CREATURE_ATTACK_CODE )
	{
		for( i = 0, index = 0; i < gpAR->ubElites; i++, index++ )
		{
			...
			wprintf( gpEnemies[index].pSoldier->name, gpStrategicString[ STR_AR_ELITE_NAME ] );
		}
		for( i = 0; i < gpAR->ubTroops; i++, index++ )
		{
			...
			gpEnemies[index].pSoldier->sSectorX = gpAR->ubSectorX;
			gpEnemies[index].pSoldier->sSectorY = gpAR->ubSectorY;
			wprintf( gpEnemies[index].pSoldier->name, gpStrategicString[ STR_AR_TROOP_NAME ] );
		}
		for( i = 0; i < gpAR->ubAdmins; i++, index++ )
		{
			...
			gpEnemies[index].pSoldier->sSectorX = gpAR->ubSectorX;
			gpEnemies[index].pSoldier->sSectorY = gpAR->ubSectorY;
			wprintf( gpEnemies[index].pSoldier->name, gpStrategicString[ STR_AR_ADMINISTRATOR_NAME ] );
		}
	...
	else
	{
		for( i = 0, index = 0; i < gpAR->ubAFCreatures; i++, index++ )
		{
			...
			gpEnemies[index].pSoldier->sSectorX = gpAR->ubSectorX;
			gpEnemies[index].pSoldier->sSectorY = gpAR->ubSectorY;
			wprintf( gpEnemies[index].pSoldier->name, gpStrategicString[ STR_AR_CREATURE_NAME ] );
		}
...
void RenderSoldierCellHealth( SOLDIERCELL *pCell )
{
	...
	if( pCell->uiFlags & CELL_RETREATED && gpAR->ubBattleStatus != BATTLE_VICTORY )
	{
		...
		wprintf( str, gpStrategicString[ STR_AR_MERC_RETREATED ] );
		pStr = str;
	}
	else if( pCell->uiFlags & CELL_RETREATING && gpAR->ubBattleStatus == BATTLE_IN_PROGRESS )
	{
		...
		{ //Retreating is shared with the status string.  Alternate between the
			...
			{ //override the health string with the retreating string.
				usColor = FONT_LTRED;
				wprintf( str, gpStrategicString[ STR_AR_MERC_RETREATING ] );
				pStr = str;
			}
		}
	}
	else if( pCell->uiFlags & CELL_SHOWRETREATTEXT && gpAR->ubBattleStatus == BATTLE_IN_PROGRESS )
	{
		...
		{
			SetFontForeground( FONT_YELLOW );
			wprintf( str, gpStrategicString[ STR_AR_MERC_RETREAT ] );
