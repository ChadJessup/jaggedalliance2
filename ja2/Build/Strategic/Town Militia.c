...
void HandleInterfaceMessageForCostOfTrainingMilitia( SOLDIERTYPE *pSoldier )
{
	...
	{
		wprintf( sString, pMilitiaConfirmStrings[ 8 ], giTotalCostOfTraining );
	...
	{
		wprintf( sString, pMilitiaConfirmStrings[ 7 ], iNumberOfSectors, giTotalCostOfTraining, pMilitiaConfirmStrings[ 1 ] );
	}
	else
	{
		wprintf( sString, L"%s%d. %s", pMilitiaConfirmStrings[ 0 ], giTotalCostOfTraining, pMilitiaConfirmStrings[ 1 ] );
	}
...
void HandleInterfaceMessageForContinuingTrainingMilitia( SOLDIERTYPE *pSoldier )
{
	...
	if( DoesSectorMercIsInHaveSufficientLoyaltyToTrainMilitia( pSoldier ) == FALSE )
	{
		...
		wprintf( sString, pMilitiaConfirmStrings[ 9 ], pTownNames[ GetTownIdForSector( sSectorX, sSectorY )], MIN_RATING_TO_TRAIN_TOWN );
	...
	if ( IsMilitiaTrainableFromSoldiersSectorMaxed( pSoldier ) )
	{
		...
		if ( bTownId == BLANK_SECTOR )
		{
			...
			wprintf( sString, pMilitiaConfirmStrings[ 10 ], sStringB, GetSectorIDString, MIN_RATING_TO_TRAIN_TOWN );
		}
		else
		{
			// town
			wprintf( sString, pMilitiaConfirmStrings[ 10 ], pTownNames[ bTownId ], MIN_RATING_TO_TRAIN_TOWN );
		}
		DoContinueMilitiaTrainingMessageBox( sSectorX, sSectorY, sString, MSG_BOX_FLAG_OK, CantTrainMilitiaOkBoxCallback );
		return;
	}
	...
	GetSectorIDString( sSectorX, sSectorY, 0, sStringB, TRUE );
	wprintf( sString, pMilitiaConfirmStrings[ 3 ], sStringB, pMilitiaConfirmStrings[ 4 ], giTotalCostOfTraining );
  ...
  if( bExitValue == MSG_BOX_RETURN_YES )
	{
		...
		else	// can't afford it
		{
			...
			wprintf( sString, L"%s", pMilitiaConfirmStrings[ 2 ] );
...
void BuildMilitiaPromotionsString( UINT16 *str )
{
	...
	{
		wprintf( pStr, gzLateLocalizedString[22], gbGreenToElitePromotions );
		wcscat( str, pStr );
		fAddSpace = TRUE;
	}
	...
	if( gbGreenToRegPromotions > 1 )
	{
		...
		wprintf( pStr, gzLateLocalizedString[23], gbGreenToRegPromotions );
		wcscat( str, pStr );
		fAddSpace = TRUE;
	}
	...
	if( gbRegToElitePromotions > 1 )
	{
		...
		wprintf( pStr, gzLateLocalizedString[24], gbRegToElitePromotions );
		wcscat( str, pStr );
	}
