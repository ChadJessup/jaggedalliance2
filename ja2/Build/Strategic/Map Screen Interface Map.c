...
void ShowTownText( void )
{
  ...
  SetFontBackground(FONT_MCOLOR_BLACK);
 	...
 	for( bTown = FIRST_TOWN; bTown < NUM_TOWNS; bTown++)
	{
		...
		{
			wprintf( sString, L"%s", pTownNames[ bTown ] );
			...
			{
				wprintf( sStringA, L"%d%%%% %s", gTownLoyalty[ bTown ].ubRating, gsLoyalString[ 0 ]);
...
//	 sTotalDistanceOfTrip = ( INT16 )DistanceToNearestRefuelPoint( )
	...
	wprintf( sString, L"%s", pHelicopterEtaStrings[ 0 ] );
...
*/
	...
	wprintf( sString, L"%d", sTotalOfTrip );
	...
	wprintf( sString, L"%d", uiTripCost );
	...
	wprintf( sString, L"%s", pHelicopterEtaStrings[ 4 ] );
	...
	wprintf( sString, L"%d%s %d%s", iTime / 60, gsTimeStrings[0], iTime % 60, gsTimeStrings[1] );
	...
	wprintf( sString, L"%d", GetNumberOfPassengersInHelicopter() );
...
*/
			...
			wprintf( sString, L"%d", iNumberOfPeopleInHelicopter );
...
void BlitMineText( INT16 sMapX, INT16 sMapY )
{
	...
	wprintf( wString, L"%s %s", pTownNames[ GetTownAssociatedWithMine( GetMineIndexForSector( sMapX, sMapY ) ) ],  pwMineStrings[ 0 ] );
	...
	{
		wprintf( wString, L"%s", pwMineStrings[ 5 ] );
		AdjustXForLeftMapEdge(wString, &sScreenX);
		mprintf( ( sScreenX - StringPixLength( wString, MAP_FONT ) / 2 ) , sScreenY + ubLineCnt * GetFontHeight( MAP_FONT ) , wString );
		ubLineCnt++;
	}
	...
	{
		wprintf( wString, L"%s", pwMineStrings[ 6 ] );
		AdjustXForLeftMapEdge(wString, &sScreenX);
		mprintf( ( sScreenX - StringPixLength( wString, MAP_FONT ) / 2 ) , sScreenY + ubLineCnt * GetFontHeight( MAP_FONT ) , wString );
		ubLineCnt++;
	}
	...
	{
		wprintf( wString, L"%s", pwMineStrings[ 7 ] );
		AdjustXForLeftMapEdge(wString, &sScreenX);
		mprintf( ( sScreenX - StringPixLength( wString, MAP_FONT ) / 2 ) , sScreenY + ubLineCnt * GetFontHeight( MAP_FONT ) , wString );
		ubLineCnt++;
	}
	...
	if (PlayerControlsMine(ubMineIndex) && !gMineStatus[ ubMineIndex ].fEmpty)
	{
		...
		wprintf( wSubString, L"%d", PredictDailyIncomeFromAMine(ubMineIndex) );
...
/*
		...
		wprintf( wSubString, L"%d", GetMaxDailyRemovalFromMine(ubMineIndex) );
		InsertCommasForDollarFigure( wSubString );
		InsertDollarSignInToString( wSubString );
		wcscat( wString, L" / ");
...
*/
		...
		{
			wprintf( wSubString, L" (%d%%%%)", (PredictDailyIncomeFromAMine(ubMineIndex) * 100 ) / GetMaxDailyRemovalFromMine(ubMineIndex) );
			wcscat( wString, wSubString );
		}
...
void DisplayLevelString( void )
{
	...
	SetFontForeground( MAP_INDEX_COLOR );
	SetFontBackground( FONT_BLACK );
	wprintf( sString, L"%s %d", sMapLevelString[ 0 ], iCurrentMapSectorZ );
...
void RenderIconsPerSectorForSelectedTown( void )
{
	...
	for( iCounter = 0; iCounter < 9; iCounter++ )
	{
		...
		SetFont( FONT10ARIAL );
		wprintf( sString, L"%d", iTotalNumberOfTroops );
...
void DrawTownMilitiaName( void )
{
	...
	wprintf( sString, L"%s %s", pTownNames[ sSelectedMilitiaTown ], pMilitiaString[ 0 ] );
	...
	wprintf( sString, L"%s %s", pTownNames[ sSelectedMilitiaTown ], pMilitiaString[ 1 ] );
